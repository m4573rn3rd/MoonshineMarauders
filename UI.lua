...
function addonTable.QuestLog:Initialize()
    ...
    self:CreateHUD() -- Create HUD on initialization
    self:UpdateHUD() -- Update HUD on initialization
end
...
function addonTable.QuestLog:CreateHUD()
    ...
    local f = CreateFrame("Frame", "MoonshineMaraudersQuestHUD", UIParent, "BackdropTemplate")
    f:SetSize(180, 60)
...
qFrame:SetScript("OnEvent", function(self, event, questID, xpReward, moneyReward)
    ...
    print("|cFF00FF00[MoonshineMarauders]|r: Quest Complete! Rewards: " .. GetCoinTextureString(moneyReward))
